import {GET_POKEMONS, GET_POKEMON_NAME, GET_POKEMONS_ID, GET_TYPES, POST_POKEMONS, FILTER, ORDER} from "../actions/actions_types";
...
const rootReducer = (state = initialState, {type, payload}) => {
    switch(type){
        ...
        case FILTER:
            if(payload === "Api"){
                const allPokemonsApi = state.pokemonsCopy.filter(pokemon => typeof pokemon.ID === "number")
                return {...state,
                        pokemons: allPokemonsApi
                }
            }
            if(payload === "Base de Datos"){
                const allPokemonsBD = state.pokemonsCopy.filter(pokemon => typeof pokemon.ID === ("string"))
                return {...state,
                        pokemons: allPokemonsBD
                }
            } else {
                return{
                    ...state,
