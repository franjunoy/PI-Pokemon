...
jest.mock('./pokemonsHandler', () => jest.fn().mockResolvedValue(mockPokemonsHandlerResponse));
...
describe('pokemonIdHandler', () => {
  ...
  it('debe devolver el resultado de findPokemonIdApi y findPokemonIdDB si se proporciona un ID de Pokémon', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ idPokemonApi: mockApiResult, idPokemonDB: mockDBResult });
  });
  ...
  it('debe devolver un error si no se encuentra ningún Pokémon con el ID proporcionado', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({ error: 'No se encontró ningún Pokémon con ese ID' });
  });
